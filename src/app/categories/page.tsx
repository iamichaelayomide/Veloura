    ...
    <div className="pb-16">
      ...
      <section className="site-shell mt-10 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
        {categories.map((category) => (
          <Link key={category.id} href={hairHref(`/categories/${category.slug}`)} className="overflow-hidden rounded-[28px] border border-white/10 bg-[#120c0b]">
            <div className="relative aspect-[5/4]">
              <Image src={category.image} alt={category.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 33vw" />
            </div>
            <div className="p-6">
              <p className="text-xs uppercase tracking-[0.3em] text-[#c58b74]">{category.accent}</p>
              <h2 className="mt-3 text-2xl text-[#f8efe8]">{category.name}</h2>
              <p className="mt-3 text-sm leading-7 text-[#bca79d]">{category.description}</p>
            </div>
          </Link>
        ))}
      </section>
